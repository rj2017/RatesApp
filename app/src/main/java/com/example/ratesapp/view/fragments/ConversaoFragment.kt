...
class ConversaoFragment : Fragment(){
    ...
    private fun handleConversao(){
        val base = spinner_val_base.selectedItem.toString()
        val referencia = spinner_val_referencia.selectedItem.toString()
        val valBase = if (edit_valor_base.text.toString() != "") edit_valor_base.text.toString().toFloat() else 1.0F
        conversaoViewModel.converter(base,referencia,valBase)
    }
    ...
    private fun observer(){
        conversaoViewModel.resultado.observe(viewLifecycleOwner, Observer {
            if (it != null){
                ...
                Toast.makeText(context,"Não localizado a moeda informada",Toast.LENGTH_SHORT).show()
            }
        })
    }
